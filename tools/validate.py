...
from kraken_sources import LEXER, ROOT, read, strip_line_comments

errors, notices = [], []

# 1. Well-formed plugin.xml, and every class it references exists.
tree = ET.parse(os.path.join(ROOT, "src/main/resources/META-INF/plugin.xml"))
...
# 2. BNF analysis.
bnf = read(os.path.join(ROOT, "src/main/bnf/Kraken.bnf"))
bnf_body = strip_line_comments(re.sub(r'/\*.*?\*/', '', bnf, flags=re.S))
...
if unused_tokens:
    notices.append(f"BNF: tokens declared but unused in the grammar: {sorted(unused_tokens)}")
